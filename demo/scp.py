...
for filename in os.listdir(folder_path):
    ...
    if filename.startswith("aaad.") and filename.endswith(".jpeg"):
        # Construct the new filename
        new_filename = filename.replace("aaad", book_name, 1)  # Replace only the first occurrence of "aaad"
    ...
    with open(coordinates_file, "r", encoding="utf-8") as file:
        ...
        for line in file:
            ...
            if len(parts) == 5:  # Ensure correct format
                ...
                if x2 - x1 <= 60 or y2 - y1 <= 60 or x1 < side or width - x1 < side or y1 < vert or height - y1 < vert:
                    continue
                
                # Collect coordinates and their respective y-values for grouping horizontally
                coordinates_data.append((label, x1-offset, y1-offset, x2+offset, y2+offset))
        ...
        for data in coordinates_data:
            ...
            if previous_y1 is None or abs(y1 - previous_y1) <= 60:  # Allow small variation for grouping
        ...
        for data in group:
            try:
               ...
               # y_offset = (48 - 40) // 2
                ...
                coordinates = f"[({x1},{y1}),({x2},{y1}),({x2},{y2}),({x1},{y2})]"
